...
```{r fig.width = 10, fig.height = 7}
plot <- ggplot(data = data, aes(x=student.status, fill=as.factor(student.status))) + coord_cartesian(ylim=c(0,nrow(data)))
...
plot <- plot + geom_bar(width=0.5) + xlab("Employer rating") + ylab("Count")
plot <- plot + theme(axis.text=element_text(size=17), axis.title=element_text(size=17))
plot <- plot + scale_fill_discrete(name="Sex", breaks=c(1, 0), labels=c("Student", "Not Student"))
